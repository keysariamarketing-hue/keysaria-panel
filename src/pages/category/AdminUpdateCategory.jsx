...
export const AdminUpdateCategory = ({ setOpen, id, getAllCategories }) => {
  const {
    handleSubmit,
    formState: { errors },
  } = useForm();
  ...
  const getSingleCategories = async () => {
    ...
    try {
      const res = await adminCategoriesById(allData);
      const { catName, image } = res.data.getCategory;
      setCatName(catName);
      setShowImage(image);
    } catch (error) {
      console.log(error);
  ...
  const updateCategories = async () => {
    const allData = { privateAxios, catName, image, id };
    try {
      setLoader(true);
      const res = await adminUpdateCategories(allData);
      if (res.status === 200) {
        getAllCategories();
        setOpen(false);
                ...
                <form onSubmit={handleSubmit(updateCategories)}>
                  <div className="bg-white py-6 rounded-xl">
                    <div className="space-y-6">
                      ...
                      <div>
                        <img src={showImage} className="w-20 h-20" alt="" />
                      </div>
                      <div>
                        <div className="flex items-center input-border py-2 px-3 rounded-md ">
                          <svg
